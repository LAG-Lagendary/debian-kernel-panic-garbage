...
def generate_music_json(model_path: str, model_name: str, output_file: str, prompt: str):
    ...
    try:
        ...
        # Создаем генератор. 
        # УБИРАЕМ max_tokens из вызова, так как некоторые (вероятно, более старые) 
        # версии llama-cpp-python его не поддерживают в этом методе.
        token_generator = llm.generate(
            full_prompt, 
            # max_tokens=max_tokens, # <--- УБИРАЕМ ЭТОТ АРГУМЕНТ
        ...
        for i, token in enumerate(token_generator):
            ...
            # Добавляем ручную остановку, так как max_tokens убран из generate()
            if i >= max_tokens - 1:
                break
