...
# ВАЖНО: Эта схема должна быть внутри acm_runner.py
    ...
    "properties": {
        "metadata": {
            ...
            "properties": {
                ...
                "composer": {"type": "string", "description": "Композитор или AI."},
                "tempo": {"type": "integer", "description": "Темп в BPM."},
        ...
        "events": {
            "type": "array",
            "description": "Список всех музыкальных событий (ноты, паузы, начало такта).",
            "items": {
                "type": "object",
                "properties": {
                    "type": {"type": "string", "enum": ["note", "rest", "chord", "bar_start"], "description": "Тип события."},
                    "part": {"type": "string", "description": "Название партии/инструмента (напр., 'Melody', 'Bass-guitar')."},
                    # Эти поля зависят от 'type':
                    "pitch": {"type": "string", "description": "Название ноты (напр., C4) или список питчей для аккорда."},
                    "duration": {"type": "number", "description": "Длительность в четвертных нотах (напр., 1.0 - четверть, 0.5 - восьмая)."},
                    "velocity": {"type": "integer", "description": "Громкость ноты (0-127, опционально)."}
                },
                # При генерации ноты/аккорда поля 'type', 'part', 'duration', 'pitch' должны быть обязательными.
...
def generate_music_json(model_path: str, model_name: str, output_file: str, prompt: str):
    ...
    try:
        ...
        # ИСПРАВЛЕНИЕ: Преобразование словаря схемы в строку JSON перед передачей
        schema_string = json.dumps(MUSIC_JSON_SCHEMA)
        json_grammar = LlamaGrammar.from_json_schema(schema_string)
        ...
        # ИСПРАВЛЕНИЕ: Переключаемся на `create_completion`
        # Этот метод надежно принимает max_tokens, temperature и grammar
        # в большинстве версий llama-cpp-python.
        token_generator = llm.create_completion(
            full_prompt, 
            max_tokens=max_tokens,      # max_tokens теперь должен работать здесь
            grammar=json_grammar,
            temperature=0.8,
            stop=["<|eot_id|>"],
            stream=True                 # Важно: получаем токены по одному
        )

        output_tokens = []
        generated_token_count = 0

        # Цикл теперь итерирует по "чанкам" (chunks)
        for chunk in token_generator:
            # Извлекаем текст токена из "чанка"
            token = chunk['choices'][0].get('text', '')
            
            # Иногда могут приходить пустые токены, пропускаем их
            if not token:
                continue
            ...
            # Используем generated_token_count
            progress = min(100, int((generated_token_count / max_tokens) * 100))
            print(f"\r[Прогресс]: {progress}% ({generated_token_count}/{max_tokens} токенов)", end="", file=sys.stderr)
            sys.stderr.flush()

            # Ручная остановка 'if i >= ...' больше не нужна,
            # так как max_tokens обрабатывается самим вызовом create_completion.
